  ...
  programs.nixvim.extraConfigLua =
    ...
    ''
      ...
      local VimMode = {
      	...
      	provider = function(self) return "  %2(" .. self.modes[self.mode] .. "%)" end,
      	hl = function(self)
      ...
      local FileNameModifer = {
      	hl = function() if vim.bo.modified then return { fg = "cyan", bold = true, force = true } end end,
      }
      ...
      local Diagnostics = {
      	...
      	{ provider = "![" },
      	{
      		provider = function(self) return self.errors > 0 and (self.Error .. self.errors .. " ") end,
      		hl = "DiagnosticError",
      	},
      	{
      ...
      local Tabpage = {
      	...
      	hl = function(self) if not self.is_active then return "TabLine" else return "TabLineSel" end end,
      }
      ...
      local TabLineOffset = {
      	condition = function(self)
      		...
      		self.winid = win
      		-- if vim.bo[bufnr].filetype == "neo-tree" then self.title = "neo-tree" return true
      		if vim.bo[bufnr].filetype == "aerial" then self.title = "aerial" return true end
      	end,
      	provider = function(self)
      	...
      	hl = function(self)
      		if vim.api.nvim_get_current_win() == self.winid then return "TablineSel" else return "Tabline" end
      	end,
      }
