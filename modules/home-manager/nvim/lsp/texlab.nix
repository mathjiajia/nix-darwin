      ...
      on_attach.__raw = ''
        function(client, bufnr)
        	...
        	for _, cmd in ipairs({
        		...
        		{ name = "TexlabCleanArtifacts",    fn = function() command_factory("Artifacts") end, desc = "Clean the artifacts" },
        		{ name = "TexlabCleanAuxiliary",    fn = function() command_factory("Auxiliary") end, desc = "Clean the auxiliary files" },
        		{ name = "TexlabChangeEnvironment", fn = buf_change_env,                              desc = "Change the environment at current position" },
        	}) do
        		vim.api.nvim_buf_create_user_command(bufnr, "Lsp" .. cmd.name, cmd.fn, { desc = cmd.desc })
        	end
        	...
        	for _, map in ipairs({
        		...
        		vim.keymap.set(map.mode or "n", map.lhs, map.fn, { buffer = bufnr, desc = map.desc })
        	end
        end
      '';
    };
