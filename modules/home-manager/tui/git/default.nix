{ lib, pkgs, ... }: {
  ...
  programs.lazygit = {
    ...
    settings = {
      git.diffRenderers = [
        # {
        #   command = ''${lib.getExe pkgs.delta} --features "$(defaults read -globalDomain AppleInterfaceStyle &> /dev/null && echo catppuccin-mocha || echo catppuccin-latte)" --paging=never'';
        # }
        {
          type = "extDiff";
